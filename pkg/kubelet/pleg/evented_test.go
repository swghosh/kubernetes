...
`
	...
	nextPodStatuses := make([]*kubecontainer.PodStatus, 2)
	// stop sandbox containers for first 2 pods
	for i := range podStatuses[:2] {
		podStatusCopy := (deepcopy.Copy(*podStatuses[i])).(kubecontainer.PodStatus)
		nextPodStatuses[i] = &podStatusCopy
		nextPodStatuses[i].ContainerStatuses[0].State = kubecontainer.ContainerStateExited

		pleg.updateRunningPodMetric(nextPodStatuses[i])
		pleg.cache.Set(nextPodStatuses[i].ID, nextPodStatuses[i], nil, time.Now())
